  ...
  return (
    <header className="sticky top-0 z-40 bg-[#FAFAFA]/95 backdrop-blur border-b border-[#E2E8F0] px-4 py-3 flex items-center justify-between">
      ...
      <div className="flex items-center gap-2.5">
        ...
        {/* Desktop (>= 1024px): Static Display because selector is in left sidebar */}
        <span className="hidden lg:inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold bg-[#F1F5F9] text-[#0F172A] border border-[#CBD5E1]">
          <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
          YA {selectedYear}
        </span>

        {/* Mobile (< 1024px): Interactive Dropdown because left sidebar is hidden */}
        <div className="lg:hidden relative inline-flex items-center">
          <select
            value={selectedYear}
            onChange={(e) => setSelectedYear(Number(e.target.value))}
            className="appearance-none bg-[#F1F5F9] hover:bg-[#E2E8F0] text-[#0F172A] font-bold text-xs pl-2.5 pr-6 py-1 rounded-full border border-[#CBD5E1] transition-colors cursor-pointer focus:outline-hidden focus:ring-1 focus:ring-[#0052FF]"
