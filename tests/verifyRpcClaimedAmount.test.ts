...
async function testWebConfirm() {
  console.log('🧪 Running RPC confirm_receipt_admin Exclusion & Claimed Amount Test...')
  ...
  try {
    // 1. Initial cleanup to prevent collisions
    ...
    if (insRecErr) throw new Error(`Insert Receipt Error: ${insRecErr.message}`)

    // 3. Insert 2 line items: item1 (RM60, included), item2 (RM40, excluded)
    ...
    if (insItemsErr) throw new Error(`Insert Line Items Error: ${insItemsErr.message}`)

    // 4. Execute RPC
    const { data: confirmed, error: rpcErr } = await supabase.rpc('confirm_receipt_admin', { p_receipt_id: receiptId })
    ...
    if (!pass) {
      throw new Error(`Assertion failed: expected claimed_amount=60, got ${confirmed.claimed_amount}`)
    }
    console.log('✅ TEST PASSED: claimed_amount is exactly RM60.00\n')
  } catch (err: any) {
    console.error('❌ TEST FAILED:', err.message)
    process.exitCode = 1
  } finally {
    // Guaranteed cleanup regardless of assertion failure or runtime throw
    await supabase.from('receipt_line_items').delete().eq('receipt_id', receiptId)
    await supabase.from('receipts').delete().eq('id', receiptId)
    console.log('🧹 Cleaned up test receipt from database.')
  }
}
